...
export const BodyMap: React.FC<BodyMapProps> = React.memo(({
  onPartClick,
  selectedPart,
  ...
  const applyColors = useCallback((hoveredId: string | null = null) => {
    ...
    INTERACTIVE_MUSCLES.forEach(muscleId => {
      ...
      elements?.forEach(el => {
        ...
        let color = useExerciseColors 
          ? getExerciseMuscleColor(volume)
          : getVolumeColor(volume, volumeThresholds, maxVolume);
        
        if (muscleColors && muscleColors.has(muscleId)) {
          color = muscleColors.get(muscleId) as string;
        }
  ...
  }, [muscleVolumes, maxVolume, selectedMuscleIds, hoveredMuscleIdsOverride, interactive, useExerciseColors, volumeThresholds, muscleColors]);
  ...
  const handleMouseOver = useCallback((e: MouseEvent) => {
    ...
    if (muscleGroup && INTERACTIVE_MUSCLES.includes(muscleGroup.id)) {
      ...
      if (hoveredMuscleRef.current === hoveredId) {
        return;
      }
  ...
  const handleMouseOut = useCallback((e: MouseEvent) => {
    const target = e.target as Element;
    const muscleGroup = target.closest('g[id]');
    const relatedTarget = (e.relatedTarget as Element)?.closest('g[id]');
    
    // Ignore if moving within the same muscle group
    if (muscleGroup && relatedTarget && muscleGroup.id === relatedTarget.id) {
      return;
    }
